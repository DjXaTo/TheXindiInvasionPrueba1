...
public class Enemigo {
    ...
    public final int ENEMIGO_TONTO = 1;       //enemigo que se mueve aleatoriamente
    ...
    public Enemigo(Juego j, int n) {
        ...
        if (Math.random() > 0.20) {
            ...
            velocidad = (VELOCIDAD_ENEMIGO_TONTO + Nivel) * juego.AltoPantalla / 1920;
            Log.i(Juego.class.getSimpleName(), "Velocidad de los enemigos tontos " + velocidad);
        } else {
            tipo_enemigo = ENEMIGO_INTELIGENTE;
            velocidad = (VELOCIDAD_ENEMIGO_INTELIGENTE + Nivel) * juego.AltoPantalla / 1920;
    ...
    public void ActualizaCoordenadas() {
        if (tipo_enemigo == ENEMIGO_INTELIGENTE) {
            ...
            if (coordenada_x > juego.AnchoPantalla - juego.enemigo_tonto.getWidth() && direccion_horizontal == 1)
                direccion_horizontal = -1;
